...
/**
 *		서로 다른 이기종 시스템간에 메시지를 교환할 때 공통 메시지 표현 방식(xml/json)이 필요함
 *		( 공용어인 (native data)를
 *		  번역 (marshalling) 해서 보내고 답장 받을때도
 *		  역번역(unmarshalling) 을 한다고 생각하면 될듯? 즉, 외국인간의 대화 과정 느낌임 )
 *			1. native data
...
class MarshallingTest {
	//	marshalling 할 때는 ObjectMapper를 사용.
