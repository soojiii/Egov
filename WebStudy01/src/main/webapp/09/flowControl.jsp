...
<pre>
	1. Request Dispatch : 페이지를 이동하기 전에 발생한 원본 요청을 그대로 전달하는 구조.
		...
		1) forward : 최종 목적지인 B에서 응답이 전송되는 구조. --> Model 2 구조를 표현할때 사용.
		2) include : A와 B가 응답에 대한 책임을 공유하는 구조. --> 체이지 모듈화 구조를 표현할때 사용.
		<%
...
<%-- 		<jsp:include page="/05/standard.jsp"></jsp:include>		 --%>
	...
	2. Redirect : 이동전에 body가 없고, 302 상태코드와 Location 헤더로만 구성된 응답이 먼저 전송되고(최초의 요청이 제거됨.)
			...
			최초의 요청이 POST 방식으로 전송되고, 서버에서 Redirect 로 이동하면, 이후 요청은 GET으로 전송되는
			P-R-G 패턴을 구현할때 사용됨. 
	<%
